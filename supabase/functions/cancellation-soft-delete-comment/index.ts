...
serve(async (req: Request): Promise<Response> => {
  ...
  const { data: comment, error: commentErr } = await service
    ...
    .select("id, cancellation_id, user_id, deleted_at")
    .eq("id", commentId)
    .maybeSingle();
  ...
  if (!admin && (comment as any).user_id !== user.id) return json(403, { error: "Forbidden" });

  if (!admin) {
    const cancellationId = (comment as any).cancellation_id;
  ...
  // Prefer full soft-delete fields if they exist; always redact message to avoid leaking content.
  const fullPayload = {
    deleted_at: nowIso,
    deleted_by: user.id,
    message: "Borttagen",
  ...
  let upd = await service.from("cancellation_comments").update(fullPayload).eq("id", commentId);

  if (upd.error) {
    const msg = String((upd.error as any)?.message || "").toLowerCase();
    // If schema doesn't have deleted_at/deleted_by yet, still redact the message.
    if (msg.includes("deleted_at") || msg.includes("deleted_by") || msg.includes("column")) {
      upd = await service.from("cancellation_comments").update({ message: "Borttagen" }).eq("id", commentId);
    }
  }
