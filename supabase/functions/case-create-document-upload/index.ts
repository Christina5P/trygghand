...
serve(async (req: Request): Promise<Response> => {
  ...
  if (!fileExt || !ALLOWED_EXT.has(fileExt)) return json(400, { error: "Unsupported file type" });
  ...
  // No customer identifiers in the path
  ...
  if (!token) return json(500, { error: "Internal server error" });

  return json(200, { ok: true, path: outPath, token, mime_type: mimeType });
});
