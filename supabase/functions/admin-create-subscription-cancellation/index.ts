...
serve(async (req: Request): Promise<Response> => {
  ...
  // Notes are free-text but allowed (content) with server-side length limit.
  if (!isUuid(payload?.customer_id)) return json(req, 400, { error: "Invalid customer_id" });
  if (payload?.subscription_id != null && !isUuid(payload.subscription_id)) return json(req, 400, { error: "Invalid subscription_id" });
  ...
  const insertPayload: Record<string, any> = {
    ...
    custom_service_name: payload.custom_service_name ?? null,
    notice_period: payload.notice_period ?? null,
    last_due_date: payload.last_due_date ?? null,
    provider_contact: payload.provider_contact ?? null,
