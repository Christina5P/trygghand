...
serve(async (req: Request): Promise<Response> => {
  ...
  let allowed = admin;
  if (!allowed) {
    // Non-admin may delete only their own uploaded docs (if v2 metadata exists)
    for (const d of docs) {
  ...
  const next = docs.map((d) => {
    if (typeof d === "string") {
      ...
      return { path: d, deleted_at: nowIso, deleted_by: user.id, uploaded_by: null, uploaded_at: null, display_name: null, mime_type: null, uploaded_by_role: null };
    }
