...
/**
 * Create a Supabase client from environment variables (Vite / CRA fallbacks).
 * If your project uses different env names, adjust the keys below.
 */
const SUPABASE_URL =
...
/**
 * uploadImages: tries to upload files to a 'valuations' storage bucket and
 * returns public URLs; falls back to object URLs if storage is not configured
 * or upload fails.
 */
const uploadImages = async (files: File[]): Promise<string[]> => {
  try {
    if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
      // Supabase not configured — fall back to object URLs
      return files.map((f) => URL.createObjectURL(f));
    }

    const bucket = supabase.storage.from("valuations");
    const urls: string[] = [];

    for (const file of files) {
      const safeName = file.name.replace(/\s+/g, "_");
      const path = `valuations/${Date.now()}-${safeName}`;

      // Supabase storage upload (v2 API)
      const { error: uploadError } = await bucket.upload(path, file as Blob, {
        cacheControl: "3600",
        upsert: false,
      });
      if (uploadError) {
        console.warn("uploadImages: upload failed, falling back to object URL", uploadError);
    ...
    console.warn("uploadImages fallback to object URLs", err);
    return files.map((f) => URL.createObjectURL(f));
  }
};

/**
 * getValuations: convenience helper to fetch valuations (same shape expected
 * by the AdminPortal component).
 */
...
// saveValuation: saves a valuation row to Supabase 'valuations' table
async function saveValuation(customerId: string | null, analysis: string, image_urls: string[]) {
  try {
    const { data, error } = await supabase
      .from('valuations')
      .insert([{ customer_id: customerId, analysis_result: analysis, image_urls }])
      .select()
      .single();
